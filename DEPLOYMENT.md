...
**Get your own free EIA key (recommended).** `render.yaml` ships with `RIL_EIA_API_KEY=DEMO_KEY`
— a shared, low-quota key that works immediately but rate-limits fast under real traffic (crude/
gas/propane will visibly fall back to "Demo/Synthetic Data" until it resets). Register a free
personal key in ~30 seconds at [eia.gov/opendata/register.php](https://www.eia.gov/opendata/register.php)
(no payment, no approval wait), then in the Render dashboard: your service → **Environment** →
edit `RIL_EIA_API_KEY` → paste your key → save (triggers an automatic redeploy).
...
Open the Vercel URL. The header should show `OK`, and the forecast chart should populate with a
badge reading either `Live Market Data` (crude/gas/propane/FX, when the EIA/frankfurter fetch
succeeded) or `Demo/Synthetic Data` (everything else, always — see MODEL_CARD.md for why five of
the nine series have no free live source anywhere). If it doesn't:
...
- If a live series shows `Demo/Synthetic Data` when you expected live, check `/api/health`'s
  `live_series_status` — `false` means the last fetch failed, almost always the shared
  `DEMO_KEY` rate limit; see "Get your own free EIA key" above.
